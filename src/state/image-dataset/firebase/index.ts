...
import {CELL_LINE_DEF_NAME_KEY, CELL_LINE_DEF_PROTEIN_KEY, CELL_LINE_DEF_STRUCTURE_KEY,} from '../../../constants';
import {CellLineDef, MetadataStateBranch} from '../../metadata/types';
import {Album} from '../../types';
import {ALBUMS_FILENAME, CELL_FEATURE_ANALYSIS_FILENAME, CELL_LINE_DEF_FILENAME} from '../constants';
...
class FirebaseRequest implements ImageDataset {
  ...
  public getCellLineData =
      () => {
        return this.getCollection(CELL_LINE_DEF_FILENAME)
              ...
              snapshot.forEach((doc: QueryDocumentSnapshot) => {
                ...
                dataset[datum[CELL_LINE_DEF_NAME_KEY]] = {
                  [CELL_LINE_DEF_STRUCTURE_KEY]:
                      datum[CELL_LINE_DEF_STRUCTURE_KEY],
  ...
  public getFeatureData =
      () => {
        return this.getCollection(CELL_FEATURE_ANALYSIS_FILENAME)
            .then((snapshot: QuerySnapshot) => {
  ...
  public getAlbumData = () => {
    return this.getCollection(ALBUMS_FILENAME)
        .then((snapshot: QuerySnapshot) => {
          const dataset: Album[] = [];
          snapshot.forEach((doc: QueryDocumentSnapshot) => {
